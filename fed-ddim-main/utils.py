...
def get_public_data(path,num, dataname = "cifar10"):
    if dataname == "cifar10":
        dataset = CIFAR10(root=path, train=True, download=False)
...
def cifar_noniid( num_users, path ="./data/cifar" ):
    ...
    dataset = CIFAR10(root=path, train=True, download=False)
    test_dataset = CIFAR10(root=path, train=False, download=False)  # 1w 张
    # dict_users = {}
    ...
    if num_users == 5:
        ...
        num_shards, num_imgs = 40, 1250  # 20 * 2500 = 5 w
...
def fmnist_noniid( num_users, path = "./data" ):
    ...
    dataset = FashionMNIST( path, train=True, download=True, transform=transforms.ToTensor())
    test_dataset = FashionMNIST( path, train=False, download=True, transform=transforms.ToTensor()) # 1w 张
    # dict_users = {}
...
# 独立同分布划分 iid
# 将数据集 划分
def get_user_data( num_users, train = True, num_list=None,dataname = 'CIFAR10'):
    if dataname == 'CIFAR10':
        dataset = CIFAR10(root="./data", train=train, download=False)
    elif dataname == "FashionMNIST":
        # dataset = FashionMNIST("./data", train=train, download=True,transform=transforms.ToTensor())
        # dataset = FashionMNIST("./data", train=train, download=False, transform=transforms.Resize(32))
...
def cifar_noniid_dirichlet( num_users,  alpha = 1.0, path ="./data/cifar"  ):
    ...
    dataset = CIFAR10(root=path,  download=False, train=True)
    test_dataset = CIFAR10(root=path,  download=False, train=False)
