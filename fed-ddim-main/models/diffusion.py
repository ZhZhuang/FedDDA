...
from einops import rearrange    # 维度变换
...
class Block(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.groupnorm = nn.GroupNorm(groups, dim) if norm else Identity()  # 分 groups进行 Norm
...
class ResidualBlock(TimestepBlock):
    def __init__(self, dim_in, dim_out, time_dim, dropout, use_global_context=False, groups=8):
        ...
        if dim_in != dim_out:
            ...
            self.shortcut = nn.Identity()   # 有什么意义
        cond_dim = time_dim
    ...
    def forward(self, x, t, y):
        ...
        """
        h = self.block1(x)  # x:64，96，28，28 = h

        # y ：64，2，384
        # Add time step embeddings
        context = y
        size = h.size(-2)   # size：28
        hidden = rearrange(h, 'b c h w -> b (h w) c')
        attn = self.cross_attn(hidden, context)     # ？？？
        # attn： 64，784，96
        attn = rearrange(attn, 'b (h w) c -> b c h w', h=size)  # 维度变化
        # attn： 64，96，28，28
        h += attn

        # t:64，384
        # 位置信息融入x
        t = self.time_emb(t)    # t:64，192
        t = rearrange(t, 'b c -> b c 1 1')  # 维度扩展 ， t:64,192,1,1
        scale_shift = t.chunk(2, dim=1)     # ？？ ， chunk用于分块， 返回 scale和 shift  64,96,1,1
...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        attn = torch.einsum("bct,bcs->bts", q * scale, k * scale)   # 爱因斯坦求和约定：用于简洁的表示乘积、点积、转置等方法。
...
class Model(nn.Module):
    ...
    def __init__(
        ...
        self.config = config
        model_channels, out_channels, channel_mult = config.model.ch, config.model.out_ch, tuple(config.model.ch_mult)
        ...
        # 为什么时间编码 和 标签编码用的 不一样，尽管都是变为 time_embed_dim
        self.label_embedding = nn.Embedding(label_num, time_embed_dim)  # 见过两次了，cGAN中也使用过一次
        self.to_time_tokens = nn.Sequential(
            nn.Linear(time_embed_dim, num_time_tokens * cond_dim),  # 维度 * 2
        ...
        for level, mult in enumerate(channel_mult):
            for _ in range(num_res_blocks):     # 每一层有 2 个残差
                layers = [
                    ResidualBlock(ch, mult * model_channels, time_embed_dim, dropout)
                ]
                ch = mult * model_channels
                if ds in attention_resolutions:     # attention_resolutions 没有元素
    ...
    def forward(self, x, t, y):
        ...
        """
        # time step embedding
        t = self.time_embed(timestep_embedding(t, self.model_channels))  # t ： 64，384
        y = self.label_embedding(y)     # ？ 64， 384
        y = self.to_time_tokens(y)      # ？ 64， 2, 384
